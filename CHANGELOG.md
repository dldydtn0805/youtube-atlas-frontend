...
- Fixed the scheduled-sell admin setting failing to load with `permission denied for table game_settings` by granting the Edge Function `service_role` only the select, insert, and update privileges required for reading and saving the singleton setting. Applied migration `20260802031000` and verified the setting read returns HTTP 200 for `service_role` while direct anonymous access remains blocked with HTTP 401.
- Added a database-backed admin setting for the scheduled-sell target profit-rate default, wired it into newly opened sell orders without overwriting user edits or existing orders, and disabled the instant-sell tab while a just-purchased holding is waiting for the next rank refresh. Applied migration `20260802030000`, deployed Supabase API v29 and Vercel production deployment `dpl_ZVvqXtWAdUNSeoYm6Urw8oRSh8F2`, and verified 474 tests, the production build, ESLint with no errors, Deno API checks, remote DB lint, API health, protected-route authentication, and the production admin login shell with no console errors. An authenticated admin mutation was not run because the available browser had no admin session.
- Reduced hourly trend collection to the three countries exposed by the app—Korea, the United States, and Japan—by unscheduling the legacy Brazil and Indonesia Cron jobs without deleting their existing trend data. Restarted the resource-exhausted Supabase project, applied migration `20260802020000`, and verified an `ACTIVE_HEALTHY` project, five active Atlas jobs, zero legacy region jobs, an up-to-date remote migration state, and a database-backed comments API response. The comments table contained only seven rows and no stored YouTube comment highlights.
- Allowed scheduled sells to be registered immediately after a purchase while keeping instant sells locked until the next rank refresh; locked holdings now open in scheduled mode, and settlement still waits for a newer trend signal before executing. Deployed API v28 and Vercel production deployment `dpl_1ChJNRuPHvci4kYbXwyaaCT81Dh7`; verified with 469 tests, a production build, ESLint with no errors, a Deno API check, API health and unauthorized-route responses, and overflow-free local and production shells with no console errors. Production asynchronous data remained pending because the Supabase project reported resource exhaustion during verification.
