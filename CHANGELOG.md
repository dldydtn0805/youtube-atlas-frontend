...
- Simplified one-unit trading language across the tutorial, inventory, buy and sell sheets, reservations, season results, and API errors by removing `전량`, quantity, per-unit, and position jargon; completed-sale logs now lead with relative time such as `3일 전` while preserving the exact sale date in a tooltip. Deployed API v27 and Vercel production deployment `dpl_8Mdjua4zAF6WAJMQHMeYL8QAVpaJ`; verified with 466 tests, a production build, ESLint with no errors, a Deno API check, and live desktop/mobile browser checks with no layout overflow or console errors.
- Removed nonessential highlight-score explanations from the first-visit tutorial, manual game guide, and tier criteria guide, and added regression checks that tutorial pricing copy stays free of premium, count, anchor, and net-order jargon; verified with 464 tests, a production build, ESLint with no errors, and desktop/mobile browser checks with no layout overflow.
- Rewrote ranking-game guide step 2 around user purchase and sale frequency, replacing count, rank-anchor, net-order, and premium jargon in both the first-visit tutorial and manual guide preview; verified with 464 tests, a production build, ESLint with no errors, and desktop/mobile browser checks with no layout overflow.
- Replaced technical trend-sync wording throughout the tutorial, manual guide, inventory sell locks, trade forms and receipts, chart descriptions, and API errors with the user-facing `순위 갱신` terminology; verified with 464 tests, a production build, ESLint with no errors, and desktop/mobile browser checks with no layout overflow.
- Unified the ranking game into one global season, wallet, inventory, tier, leaderboard, notification feed, and season history while keeping country charts as independent markets for each position's live rank and price; subscribed the shared portfolio to every supported market's realtime updates, migrated active country wallets with only one starting-balance baseline, preserved positions and transaction records, deployed database migration `20260802010000` and API v25, and verified 464 tests, a production build, ESLint with no errors, Deno checks, one active `GLOBAL` season, and live KR/US/JP market responses.
