...
vi.mock("../../../components/VideoPlayer/VideoPlayer", () => ({
  default: ({ isCinematic }: { isCinematic?: boolean }) => (
    <div
      ...
      data-cinematic={isCinematic ? "true" : "false"}
...
function createProps(
  overrides: Partial<React.ComponentProps<typeof PlayerStage>> = {},
): React.ComponentProps<typeof PlayerStage> {
  return {
...
describe("PlayerStage", () => {
  it("shows the cinematic toggle on desktop layouts", () => {
    ...
    expect(
      screen.getByRole("button", { name: "시네마틱 모드" }),
    ).toBeInTheDocument();
  });
  ...
  it("hides the cinematic toggle on mobile layouts", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "시네마틱 모드" }),
  ...
  it("hides mobile game summary buttons for anonymous users", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "내 게임 열기" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "지갑 현황 열기" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "티어 현황 열기" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps the mobile panel layout while only the docked video player uses the cinematic path", () => {
    ...
    expect(
      container
        .querySelector(".app-shell__stage")
        ?.getAttribute("data-cinematic"),
    ).toBe("false");
    expect(
      container
        .querySelector(".app-shell__stage-stack")
        ?.getAttribute("data-cinematic"),
    ).toBe("false");
    expect(
      container
        .querySelector(".app-shell__panel--player")
        ?.getAttribute("data-cinematic"),
    ).toBe("false");
  ...
  it("keeps the watch layout inactive outside desktop cinematic mode", () => {
    ...
    expect(container.querySelector(".app-shell__watch-layout")).toHaveAttribute(
      "data-active",
      "false",
    );
