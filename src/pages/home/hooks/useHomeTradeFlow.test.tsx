...
describe('useHomeTradeFlow scheduled sell lock split', () => {
  ...
  it('uses the admin-configured profit rate as the scheduled-sell default', async () => {
    const createScheduledSellOrder = vi.fn().mockResolvedValue({});
    ...
    expect(result.current.scheduledSellTargetProfitRatePercent).toBe(25.5);

    act(() => {
      result.current.setScheduledSellTriggerType('PROFIT_RATE');
    });
