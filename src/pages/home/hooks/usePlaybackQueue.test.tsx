...
describe("usePlaybackQueue", () => {
  ...
  it("prefers the configured initial playback section when auto-selecting", async () => {
    const randomSpy = vi.spyOn(Math, "random").mockReturnValue(0.999);
    const setSelectedCategoryId = vi.fn();
    const buyableSection = createSection(
      "buyable-market",
      createVideoIds(60, "video-buyable"),
    );
  ...
  it("applies the preferred initial section once when a selection key is provided", async () => {
    ...
    act(() => {
      result.current.handleSelectVideo(
        "video-top",
        getCategoryPlaybackQueueId("0"),
      );
    });
  ...
  it("falls back to the first portfolio video when the keyed preferred section is empty", async () => {
    ...
    const portfolioSection = createSection(GAME_PORTFOLIO_QUEUE_ID, [
      "video-latest-position",
    ]);
    ...
    await waitFor(() => {
      expect(result.current.activePlaybackQueueId).toBe(
        GAME_PORTFOLIO_QUEUE_ID,
      );
  ...
  it("waits for the preferred initial playback section before falling back to the chart", async () => {
    ...
    preferredInitialPlaybackSection = createSection("buyable-market", [
      "video-buyable",
    ]);
  ...
  it("auto-selects the first video after a user changes category", async () => {
    ...
    let selectedSection = createSection(getCategoryPlaybackQueueId("0"), [
      "video-top",
    ]);
    ...
    selectedSection = createSection(getCategoryPlaybackQueueId("10"), [
      "video-music",
    ]);
    rerender();
  ...
  it("waits for the selected category chart instead of auto-selecting an owned video", async () => {
    ...
    const { result, rerender } = renderHook(() =>
      usePlaybackQueue({
        likedVideoSection: undefined,
        gamePortfolioSection: createSection(GAME_PORTFOLIO_QUEUE_ID, [
          "video-owned",
        ]),
        isMobileLayout: false,
        realtimeSurgingSection: undefined,
  ...
  it("keeps a history playback selection instead of falling back to the first chart video", async () => {
    ...
    const { result } = renderHook(() =>
      usePlaybackQueue({
        likedVideoSection: undefined,
        historyPlaybackSection: createSection(HISTORY_PLAYBACK_QUEUE_ID, [
          "video-history",
        ]),
        isMobileLayout: false,
        realtimeSurgingSection: undefined,
  ...
  it("keeps a selected extra-section video when that queue refreshes without it", async () => {
    const setSelectedCategoryId = vi.fn();
    let buyableSection = createSection("buyable-market", [
      "video-buyable",
      "video-next",
    ]);
  ...
  it("keeps a selected portfolio video when the portfolio queue refreshes empty", async () => {
    const setSelectedCategoryId = vi.fn();
    let portfolioSection = createSection(GAME_PORTFOLIO_QUEUE_ID, [
      "video-owned",
    ]);
