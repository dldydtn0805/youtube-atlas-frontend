...
describe("home utils", () => {
  ...
  it("builds a realtime surging section for the all category", () => {
    expect(
      ...
      description:
        "전체 차트에서 직전 집계 대비 순위가 5계단 이상 오른 영상을 모았습니다.",
      items: [
  ...
  it("keeps the original order for popularity descending", () => {
    const section = createVideoSection("0", "전체", [
      "video-1",
      "video-2",
      "video-3",
    ]);
    ...
    expect(
      sortVideoSection(section, "popular-desc")?.items.map((item) => item.id),
  ...
  it("reverses the original order for popularity ascending", () => {
    ...
    expect(
      sortVideoSection(section, "popular-asc")?.items.map((item) => item.id),
    ).toEqual(["video-3", "video-2", "video-1"]);
  });
  ...
  it("sorts a video section by view count descending", () => {
    ...
    expect(
      sortVideoSection(section, "views-desc")?.items.map((item) => item.id),
  ...
  it("sorts a video section by view count ascending", () => {
    ...
    expect(
      sortVideoSection(section, "views-asc")?.items.map((item) => item.id),
  ...
  it("sorts a video section by rank rise magnitude while keeping non-rising videos last", () => {
    ...
    expect(
      sortVideoSection(section, "rank-up")?.items.map((item) => item.id),
    ).toEqual(["video-3", "video-1", "video-2", "video-4"]);
  });
  ...
  it("sorts a video section by rank drop magnitude while keeping non-falling videos last", () => {
    ...
    expect(
      sortVideoSection(section, "rank-down")?.items.map((item) => item.id),
    ).toEqual(["video-2", "video-3", "video-1", "video-4"]);
  });

  it("falls back to smaller rise magnitude for rank drop sorting when a section has no falling videos", () => {
    const section = createVideoSection("realtime-surging", "실시간 급상승", [
      "video-1",
      "video-2",
      "video-3",
    ]);
  ...
  it("formats the selected video overall rank label", () => {
    ...
    expect(
      formatSelectedVideoRankLabel("대한민국", 137, { chartOut: true }),
    ).toBe("차트 아웃");
  ...
  it("formats current trend ranks before falling back to loading or missing states", () => {
    ...
    expect(formatTrendRankLabel(undefined, true)).toBe("차트 아웃");
  });
    ...
    expect(
      resolvePlaybackCategoryLabel({
        ...
        extraPlaybackSections: [
          createVideoSection("chart:music", "음악", ["video-1"]),
        ],
        fallbackLabel: "전체",
        selectedPlaybackSection: createVideoSection("category:0", "TOP 200", [
          "video-2",
        ]),
  ...
  it("infers the current video label for portfolio and history playback queues", () => {
    expect(
      resolvePlaybackCategoryLabel({
        ...
        fallbackLabel: "전체",
        realtimeSurgingSection: createVideoSection(
          REALTIME_SURGING_QUEUE_ID,
          "실시간 급상승",
          ["video-1"],
        ),
    ...
    expect(
      resolvePlaybackCategoryLabel({
        ...
        extraPlaybackSections: [
          createVideoSection("chart:music", "음악", ["video-2"]),
        ],
        fallbackLabel: "전체",
        selectedPlaybackSection: createVideoSection("category:0", "TOP 200", [
          "video-2",
