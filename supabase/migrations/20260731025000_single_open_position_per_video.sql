create or replace function public.enforce_one_open_position_per_video()
returns trigger
language plpgsql
...
begin
  if new.status = 'OPEN' then
    perform pg_advisory_xact_lock(
      hashtextextended(
        new.season_id::text || ':' || new.user_id::text || ':' || new.video_id,
        0
      )
    );
...
drop trigger if exists game_positions_one_open_video_insert on public.game_positions;
create trigger game_positions_one_open_video_insert
before insert on public.game_positions
...
revoke all on function public.enforce_one_open_position_per_video()
