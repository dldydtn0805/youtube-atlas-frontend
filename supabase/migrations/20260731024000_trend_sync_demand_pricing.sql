alter table public.video_trend_signals
  add column if not exists sync_buy_count integer not null default 0
    check (sync_buy_count >= 0),
  add column if not exists sync_buy_quantity bigint not null default 0
    check (sync_buy_quantity >= 0);

alter table public.game_seasons
  alter column starting_balance_points set default 100000;

update public.game_seasons
set starting_balance_points = 100000
where status = 'ACTIVE';
