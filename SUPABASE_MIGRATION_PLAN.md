...
| P1       | Reliable OAuth callback recovery                            | Done: Google login and every contextual YouTube authorization return to the current canonical page so React Router cannot erase Supabase's callback session on the root redirect                                           |
...
| P1       | Single-name home branding                                   | Done: the two-line YouTube Atlas and the rank game header is replaced by a polished, responsive The Rank Game logo with matching accessible navigation text                                                                |
