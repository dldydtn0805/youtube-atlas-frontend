...
37. Fixed the production settings load failure reproduced as `permission denied for table game_settings` by granting `service_role` the table's select, insert, and update privileges in migration `20260802031000`; direct service-role reads and the protected Edge API contract are reverified after applying the migration.
